...
export class ApiService {
	...
	public makeRequest(type: 'GET' | 'POST' | 'DELETE' | 'PUT', options: ApiRequestInterface): any {
	...
	private sendRequest(type: 'GET' | 'POST' | 'DELETE' | 'PUT', options: ApiRequestInterface): any {
		...
		switch (type) {
			...
			default: {
				return new Error(`This method is not accepted for this API Route ${url}`);
			}
		}
	}
	...
	private post(url: string, body: any | null, params: HttpParams): Observable<Object> {
		const headers = this.global.headers;
		const options = {
			headers,
			params
	...
	private put(url: string, body: any | null, params: HttpParams): Observable<Object> {
		const headers = this.global.headers;
		const options = {
			headers,
			params
